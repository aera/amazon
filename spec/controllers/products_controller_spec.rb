...
RSpec.describe ProductsController, type: :controller do
  ...
  let(:product)   {FactoryGirl.create(:product, category: category, user_id: user.id)}
  let(:product_1) {FactoryGirl.create(:product, category: category, user_id: user.id)}
  ...
  describe "new" do
    context "with users not signed in" do
    ...
    context "with user signed in" do
      context "with non owner signed in" do
        ...
        it "sets a flash alert message" do
          expect(flash[:alert]).to eq(nil)          #set to nil as the page now cannot be reached due to user validation
        end

        it "responds with an error page" do  #"redirects back to the product page"
